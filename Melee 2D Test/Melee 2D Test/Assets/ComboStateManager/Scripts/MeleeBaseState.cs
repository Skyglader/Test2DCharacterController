...
public class MeleeBaseState : State
{
    ...
    public override void OnEnter(StateMachine _stateMachine)
    {
        ...
        animator = GetComponent<Animator>(true);
    ...
    public override void OnUpdate()
    {
        ...
        // Check if the attack window is open
        if (animator.GetFloat("AttackWindowOpen") > 0f)
        {
            // If attack button is pressed during the open window, set the timer
            if (Input.GetMouseButtonDown(0))
            {
                AttackPressedTimer = 3f; // Set a reasonable time frame
                shouldCombo = true; // Allow combo continuation
            }
        }
        

        // If the timer runs out, prevent combo continuation
